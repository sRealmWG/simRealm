simulation_v8_steps_future <- function(strategy = "multisession", nyears = 5L, seed, simulation_ID) {
   library("future")
   library("mobsim")
   library("sRealmTools")
   ...
   for (param_i in 1:nrow(parameter_table))  res[[param_i]] %<-% {
      ...
      for (timestep in 1:nyears) {
         comm <- sRealmTools::jitter_species(comm = comm,
                                             sd = parameter_table$MOVEMENT_SD[param_i],
                                             drift = parameter_table$DRIFT[param_i]
         )
         comm <- sRealmTools::torusify(comm)
         res_param_i[[timestep]] <- mobsim::sample_quadrats(comm = comm, n_quadrats = parameter_table$N_QUADRATS[param_i], quadrat_area = parameter_table$QUADRAT_AREA[param_i], method = "grid", plot = FALSE)$spec_dat[55L, ]
      }
      res[[param_i]] <- res_param_i
   } %seed% seed %packages% "sRealmTools"
   Sys.time() - beginning
   res <- as.list(res)
   Sys.time() - beginning
   saveRDS(object = res, file = paste0("./data/simulations/mobsim/temporary_files/", simulation_ID, "_sim_steps_8.rds"))
   return(res)
}
