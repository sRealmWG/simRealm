...
png_path <- file.path(tempdir(), "r_frame%03d.png")
png(png_path)
par(ask = FALSE)
...
graphics::rect(
   xleft = mean(c(0, 1)) - sqrt(c(0.1, 0.01)) / 2,
   ybottom = mean(c(0, 1)) - sqrt(c(0.1, 0.01)) / 2,
   xright = mean(c(0, 1)) + sqrt(c(0.1, 0.01)) / 2,
   ytop = mean(c(0, 1)) + sqrt(c(0.1, 0.01)) / 2,
   lwd = 2
)
...
gif_file <- paste0("./figures/communities/", simulation_ID, "_replacement.gif")
