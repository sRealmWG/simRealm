...
## meta data
# TimeSeriesID level (alpha level)
meta_TimeSeriesID <- ft_filtered %>%
  dplyr::select(SourceID, TimeSeriesID, Protocol:Waterbody) %>%
  distinct()

# SourceID level (gamma level)
meta_SourceID <- ft_4loc_filtered %>% 
  dplyr::select(SourceID,  Latitude, Longitude, all_site, min_site,  n_years, duration) %>%
    distinct() %>%
  group_by(SourceID) %>%
  mutate(cent_latitude = mean(Latitude),
         cent_longitude = mean(Longitude)) %>% 
  ungroup() %>%
  dplyr::select(-c(Latitude, Longitude)) %>% 
  distinct() %>%
  rename(min_site_eachYear = min_site)

# remove no needed columns
ft_filtered <- ft_4loc_filtered %>% dplyr::select(-(n_site:duration)) 

# save data
write_csv(meta_TimeSeriesID, file = "RivFishTIME_meta_TimeSeriesID_alphaLevel.csv")
write_csv(meta_SourceID, file = "RivFishTIME_meta_meta_SourceID_gammaLevel.csv")
save(ft_filtered, meta_TimeSeriesID, meta_SourceID, file = "RivFishTIME_4sites_10years_filtered.RDATA")
